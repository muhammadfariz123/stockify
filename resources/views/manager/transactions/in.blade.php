...
@section('content')
    ...
    <div class="bg-white p-6 rounded-xl shadow-sm border mb-8">
        <h2 class="text-xl font-semibold mb-4">{{ isset($editTransaction) ? 'Edit Transaksi' : 'Tambah Transaksi' }}</h2>
        <form action="{{ route('manager.transactions.store.in') }}" method="POST">
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="product_id" class="w-full border p-2 rounded" required>
                        @foreach ($products as $product)
                            <option value="{{ $product->id }}" {{ (isset($editTransaction) && $editTransaction->product_id == $product->id) ? 'selected' : '' }}>{{ $product->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="quantity" class="w-full border p-2 rounded" value="{{ $editTransaction->quantity ?? '' }}" required>
                </div>
                ...
                <div>
                    ...
                    <select name="supplier_id" class="w-full border p-2 rounded" required>
                        @foreach ($suppliers as $supplier)
                            <option value="{{ $supplier->id }}" {{ (isset($editTransaction) && $editTransaction->supplier_id == $supplier->id) ? 'selected' : '' }}>{{ $supplier->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="mt-4">
                ...
                @if (isset($editTransaction))
                    <a href="{{ route('manager.transactions.in') }}" class="ml-2 text-gray-600 hover:underline">Batal</a>
                @endif
            </div>
        </form>
    </div>
                ...
                <tbody>
                    @foreach ($transactionsIn as $transaction)
                        <tr class="border-b">
                            ...
                            <td class="p-2">{{ $transaction->transaction_date ?? $transaction->created_at->format('d-m-Y') }}</td>
                            <td class="p-2">
                                <a href="{{ route('manager.transactions.edit.in', $transaction->id) }}" class="text-blue-600 hover:underline">Edit</a>
                                <form action="{{ route('manager.transactions.delete.in', $transaction->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-600 ml-2 hover:underline" onclick="return confirm('Yakin hapus transaksi ini?')">Hapus</button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
